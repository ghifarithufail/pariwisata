    ...
    <div class="card text-center">
        <div class="card-header d-flex justify-content-between align-items-center">
            <div class="search-field">
                <form action="{{ route('kondektur') }}" method="GET">
                    <input type="text" name="search" class="form-control" placeholder="Search...">
                </form>
            </div>
            <div>
                <a href="{{ route('kondektur') }}" class="m-0"><h5> Daftar kondektur</h5></a>
                <p class="m-0">Total : {{ App\Models\Hrd\Kondektur::count() }} </p>
            </div>
            <div class="add-new-role">
                <!-- Tombol "Add New Role" -->
                <a href="{{ route('kondektur/create') }}" class="btn btn-primary mb-2 text-nowrap" target="_blank">
                    + kondektur
                </a>
            </div>
        </div>
