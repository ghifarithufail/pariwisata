    ...
    <div class="card text-center">
        <div class="card-header d-flex justify-content-between align-items-center">
            <div class="search-field">
                <form action="{{ route('pengemudi') }}" method="GET">
                    <input type="text" name="search" class="form-control" placeholder="Search...">
                </form>
            </div>
            <div>
                <a href="{{ route('pengemudi') }}" class="m-0"><h5> Daftar pengemudi</h5></a>
                <p class="m-0">Total : {{ App\Models\Hrd\pengemudi::count() }} </p>
            </div>
            <div class="add-new-role">
                <!-- Tombol "Add New Role" -->
                <a href="{{ route('pengemudi/create') }}" class="btn btn-primary mb-2 text-nowrap" target="_blank">
                    + pengemudi
                </a>
            </div>
        </div>
